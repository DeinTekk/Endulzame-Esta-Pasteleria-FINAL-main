...
export default function TarjetaProducto({ producto, onProductoClick }: TarjetaProductoProps) {
  ...
  const calificacionPromedio = resenas.length > 0
    ...
    : 0;
  const textoResena = resenas.length > 0 ? `(${resenas.length})` : '';
  ...
  const unidadTexto = producto.unidad && producto.stock !== 1 && unidadesPluralizables.includes(producto.unidad)
    ? `${producto.unidad}s`
  ...
  return (
    <div className="col-sm-6 col-md-4 mb-4">
      ...
      >
        <img
          src={producto.imagen && producto.imagen.startsWith('img/') ? `/${producto.imagen}` : producto.imagen}
          alt={producto.nombre}
        ...
        <div className="card-body d-flex flex-column">
          ...
          <p className="card-text text-center descripcion-producto">
            {producto.descripcion ? producto.descripcion.substring(0, 70) : ''}...
          </p>

          <div className="mt-auto d-flex justify-content-between align-items-center pt-2">
            {producto.precioConDescuento !== undefined && producto.precioConDescuento !== null ? (
              <div>
